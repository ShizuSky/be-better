...
class Database:
    ...
    def add_habit(self, name):
        ...
        self.cursor.execute(
            "INSERT INTO habits (name, streak, last_completed) VALUES (?, 0, ?)",
            (name, now_str),
        )
        self.conn.commit()
    ...
    def complete_habit(self, habit_id):
        now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self.cursor.execute(
            "UPDATE habits SET streak = 0, last_completed = ? WHERE id = ?",
            (now_str, habit_id),
        )
        self.conn.commit()
        return True
    ...
    def toggle_task(self, task_id, current_state):
        ...
        self.cursor.execute(
            "UPDATE tasks SET is_done = ? WHERE id = ?", (new_state, task_id)
        )
        self.conn.commit()
    ...
    def add_workout_entry(
        self, session_id, ex_name, sets, reps, weight, workout_date
    ):
    ...
    def get_workout_details(self, session_id):
        ...
        self.cursor.execute(
            "SELECT exercise_name, sets, reps, weight FROM workouts WHERE session_id = ?",
            (session_id,),
        )
        return self.cursor.fetchall()
